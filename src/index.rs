...
pub type Sources = BTreeMap<String, Entries>;
...
impl<'a> IntoIterator for &'a Entries {
	...
	fn into_iter(self) -> Self::IntoIter {
		match self {
			Entries::Directory(entries) => Either::Left(entries.into_iter()),
...
fn group_by_source(entries: Vec<SourceEntry>) -> Sources {
	...
	for source_entry in entries {
		...
		if source.is_file {
			let prev_value = sources.insert(source.id.to_string(), Entries::File(entry));
			assert!(prev_value.is_none(), "Sources should be unique and file sources should only have one entry");
			continue;
		}
		
		if let Some(source_entries) = sources.get_mut(&*source.id) {
			...
			sources.insert(source.id.to_string(), Entries::Directory(vec![entry]));
		}
	}
